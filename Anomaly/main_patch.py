...
from network_patch import Generator, vgg_layers
...
def train_step(noise_images, images,
    ...
    with tf.GradientTape() as disc_tape:
        ...
        disc_loss = autoencoder_loss(images, generated_images) - ssim_loss()
...
def train(dataset, epochs, generator, discriminator, discriminator_optimizer, class_num):
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch in dataset.train_ds:
            ...
            image_patch_0 = make_patch_image(data_batch, 32, 16)
            noise_image_patch_0 = make_patch_image(noise_data_batch, 32, 16)
            image_patch_1 = make_patch_image(image_1, 32, 8)
            noise_image_patch_1 = make_patch_image(noise_image_1, 32, 8)
            image_patch_2 = make_patch_image(image_2, 32, 4)
            noise_image_patch_2 = make_patch_image(noise_image_2, 32, 4)
            image_patch_3 = make_patch_image(image_3, 32, 2)
            noise_image_patch_3 = make_patch_image(noise_image_3, 32, 2)

            loss_0, gen_img_0 = train_step(noise_image_patch_0[:, :, :], image_patch_0[:, :, :],
                                           noise_image_patch_1[:, :, :], image_patch_1[:, :, :],
                                           noise_image_patch_2[:, :, :], image_patch_2[:, :, :],
                                           noise_image_patch_3[:, :, :], image_patch_3[:, :, :],
                                           generator, discriminator, discriminator_optimizer)
        ...
        for x_batch, y_batch in dataset.test_ds:
            ...
            image_patch_0 = make_patch_image(data_batch, 32, 16)
            image_patch_1 = make_patch_image(data_batch_ds_1, 32, 8)
            image_patch_2 = make_patch_image(data_batch_ds_2, 32, 4)
            image_patch_3 = make_patch_image(data_batch_ds_3, 32, 2)

            gen_img_0, gen_img_1, gen_img_2, gen_img_3 = test_step(image_patch_0[:, :, :],
                                                                   image_patch_1[:, :, :],
                                                                   image_patch_2[:, :, :],
                                                                   image_patch_3[:, :, :],
            ...
            for k in range(1):
                ...
                for i in range(4):
                    if i == 0:
                        ...
                        for n in range(16):
                            for m in range(16):
                                ge_img[0, m * 32:m * 32 + 32, n * 32:n * 32 + 32, 0] = gen_img_0[16 * n + m, :, :, 0]
                    elif i == 1:
                        in_img = data_batch_ds_1
                        ge_img = np.zeros([1, 256, 256, 1])
                        for n in range(8):
                            for m in range(8):
                                ge_img[0, m * 32:m * 32 + 32, n * 32:n * 32 + 32, 0] = gen_img_0[8 * n + m, :, :, 0]
                    elif i == 2:
                        in_img = data_batch_ds_2
                        ge_img = np.zeros([1, 128, 128, 1])
                        for n in range(4):
                            for m in range(4):
                                ge_img[0, m * 32:m * 32 + 32, n * 32:n * 32 + 32, 0] = gen_img_0[4 * n + m, :, :, 0]
                    elif i == 3:
                        in_img = data_batch_ds_3
                        ge_img = np.zeros([1, 64, 64, 1])
                        for n in range(2):
                            for m in range(2):
                                ge_img[0, m * 32:m * 32 + 32, n * 32:n * 32 + 32, 0] = gen_img_0[2 * n + m, :, :, 0]

                    diff_img = np.abs(in_img[k, :, :, 0] - (ge_img[k, :, :, 0] + 1.) / 2.)
                    threshold = 1 - (np.mean(diff_img) + 2.5 * np.std(diff_img))
                    threshold = 0.97
                    q_diff_img = ((diff_img + threshold).astype(np.int8)).astype(np.float32)

                    if not no_figure:
                        ax[i, 0].imshow(in_img[k, :, :, 0], cmap='gray')
                        ax[i, 1].imshow((ge_img[k, :, :, 0] + 1.) / 2., cmap='gray')
                ...
                if not no_figure:
                    ax[0, 5].imshow(label_batch[k, :, :, 0], cmap='gray', vmin=0, vmax=1)
                ...
                total_iou += iou(label_batch[k, :, :, 0], mask)
                total_num += 1
                ...
                if not no_figure:
                    fig.savefig('./class_%d_v2/epoch_%d_%d_%d_result.png' % (class_num, epoch + 1, test_itr_num, k))
