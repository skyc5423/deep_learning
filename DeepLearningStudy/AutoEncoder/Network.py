...
class AutoEncoder(tf.keras.Model):
    def __init__(self, name):
        ...
        self.optimizer = tf.keras.optimizers.Adam(
            learning_rate=0.0001, beta_1=0.5, epsilon=1e-07, amsgrad=False,
        )
    ...
    def __call__(self, input_batch, training=True):
        ...
        feature_fc_1 = self.relu_fc_1(self.fc_1(reshape))
        feature_fc_2 = self.relu_fc_2(self.fc_2(feature_fc_1))
    ...
    def train_autoencoder(self, input_image):
        with tf.GradientTape() as tape:
            tape.watch(self.trainable_variables)

            generated_image = self(input_image)

            loss_total = self.loss_l2_norm(input_image, generated_image)
    ...
    def train_(self, input_data):
        ...
        loss_total = self.train_autoencoder(input_data)
