...
class GANTrainer(object):
    ...
    def train_discriminator(self, input_batch, input_noise):
        with tf.GradientTape() as tape:
            tape.watch(self.discriminator.trainable_variables)

            generated_image = self.generator(input_noise)
            dis_real = self.discriminator(input_batch)
            dis_fake = self.discriminator(generated_image)
    ...
    def train_(self, input_batch, input_noise):
        loss_generator = self.train_generator(input_noise)
        loss_discriminator = self.train_discriminator(input_batch, input_noise)
